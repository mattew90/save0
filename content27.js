...
const upscaledCanvasCache = new Map();
async function replaceWithSincCanvas(img, scaleX, scaleY, width, height, style = null) {
  ...
  // Caching key: src + desired width/height/scale for this instance
  const cacheKey = [
  ...
  let canvas = upscaledCanvasCache.get(cacheKey);
  ...
  if (!canvas) {
    ...
    if (gl.getUniformLocation(prog, "uDown")) {
      gl.uniform1i(gl.getUniformLocation(prog, "uDown"), (scaleX < 1 && scaleY < 1) ? 1 : 0);
    }
    ...
    upscaledCanvasCache.set(cacheKey, canvas);
    // Log on success
  ...
  // Insert (clone) or move this canvas after the current img
  // If the canvas is already inserted after the img, leave it. Otherwise, insert a clone.
  // This avoids DOM reordering bugs, and allows multiple identical images to reuse the same upscaled data.
  let sibling = img.nextSibling;
  if (sibling !== canvas) {
    // If canvas is *already* after another img, insert a clone here
    if (canvas.parentNode && canvas.parentNode !== img.parentNode) {
      // We need a new canvas for this parent!
      let clone = canvas.cloneNode(true);
      img.parentNode.insertBefore(clone, img.nextSibling);
    } else if (canvas.parentNode !== img.parentNode || canvas.previousSibling !== img) {
...
// --- Only show all upscaled canvases, reusing the same data for same src/size images ---
async function processImages() {
  ...
  for (const img of imgs) {
    ...
    // --- YouTube thumbnail lazy loading PATCH ---
    if (
      (img.currentSrc || img.src) &&
      (/\/\/i\.ytimg\.com\/vi\//.test(img.currentSrc || img.src) ||
       /\/\/yt3\.ggpht\.com\//.test(img.currentSrc || img.src)) &&
      (img.naturalWidth < 50 || img.naturalHeight < 50)
    ) {
      // Placeholder or not loaded yet, wait for real image
      continue;
    }
    if (!isImageReady(img)) {
