...
// Handles upscaling and downscaling, logs all events, skips SVG, and robust fallback.
...
function getScaleInfo(img) {
  ...
  return { scaleX, scaleY, needs, width, height };
}
...
async function replaceWithSincCanvas(img, scaleX, scaleY, width, height, radius = 3) {
  if (isSVG(img)) {
    console.log('[SincUpscale] Skipping SVG image:', img.src);
    return;
  }
  if (!img.complete || !img.naturalWidth) {
    img.addEventListener('load', () => replaceWithSincCanvas(img, scaleX, scaleY, width, height, radius), { once: true });
...
function processImages() {
  for (const img of document.querySelectorAll('img')) {
    ...
    const { scaleX, scaleY, needs, width, height } = getScaleInfo(img);
    if (!needs) {
    ...
    replaceWithSincCanvas(img, scaleX, scaleY, width, height, 3);
  }
}
