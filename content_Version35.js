...
function getCacheKey(img) {
  ...
  const width = Math.round(parseFloat(style.width) || img.width);
  const height = Math.round(parseFloat(style.height) || img.height);
  return `${img.currentSrc || img.src}|${width}|${height}`;
}
...
function isYouTubeThumbnail(img) {
  const src = img.currentSrc || img.src || '';
  return /\/\/i\.ytimg\.com\/vi\//.test(src) || /\/\/yt3\.ggpht\.com\//.test(src);
}
...
function copyAllComputedStyles(from, to) {
  ...
  for (let prop of computed) {
    ...
    try { to.style[prop] = computed.getPropertyValue(prop); } catch (e) {}
  }
...
function getPassthroughShaders(isWebGL2) {
  return {
    ...
    frag: isWebGL2 ? `#version 300 es
      precision highp float; in vec2 vTex; uniform sampler2D uTex; out vec4 fragColor;
      void main() { fragColor = texture(uTex, vTex); }
    ` : `
      precision highp float; varying vec2 vTex; uniform sampler2D uTex;
      void main() { gl_FragColor = texture2D(uTex, vTex); }
    `
  };
}

// --- Main upscaling, caching, reuse logic ---
async function replaceWithSincCanvas(img, scaleX, scaleY, width, height, style = null) {
  ...
  if (isYouTubeThumbnail(img) && (img.naturalWidth < 50 || img.naturalHeight < 50)) return;

  // CORS: set crossOrigin before load for remote images
  if (
    !img.src.startsWith("data:") &&
    !img.src.startsWith("blob:") &&
    !img.src.startsWith(window.location.origin) &&
  ...
  // --- Caching logic ---
  let cacheKey = getCacheKey(img);
  if (processedImagesCache.has(cacheKey)) {
    let cachedCanvas = processedImagesCache.get(cacheKey);
    let clone = cachedCanvas.cloneNode(true);
  ...
  // --- Resample and cache first occurrence ---
  ...
  img.dataset.sincUpscaled = "true";
  processedImagesCache.set(cacheKey, canvas);

  // --- WebGL2 context and fallback (same as your original logic) ---
  ...
  const fragSource = isWebGL2 ? `#version 300 es
    ...
    void main() {
      ...
      for (int dx = -3; dx <= 3; ++dx) {
        ...
        if (uDown) texel.rgb = pow(texel.rgb, vec3(2.2));
        float weight = lanczos(float(dx), r) * lanczos(float(dy), r);
        color += texel * weight; total += weight;
      }
      color /= total;
      if (uDown) color.rgb = pow(color.rgb, vec3(1.0/2.2));
      fragColor = color;
    }
  ` : `
    precision highp float;
  ...
  try {
    ...
    canvas.remove(); img.style.display = "";
    oldNearestNeighborFallback(img, scaleX, scaleY);
    return;
  }
  ...
  if (gl.getUniformLocation(prog, "uDown")) gl.uniform1i(gl.getUniformLocation(prog, "uDown"), isDownsample ? 1 : 0);
  if (isWebGL2 && useFloatFbo) gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
...
// --- Main processImages: upsample first occurrence, reuse for others ---
async function processImages() {
  for (const img of document.querySelectorAll('img')) {
    if (img.dataset.sincUpscaled === "true") continue;
    if (isSVG(img)) { img.dataset.sincUpscaled = "svg"; continue; }
    if (isYouTubeThumbnail(img) && (img.naturalWidth < 50 || img.naturalHeight < 50)) continue;
    if (!isImageReady(img)) {
    ...
    if (!needs) { img.dataset.sincUpscaled = "no"; continue; }
