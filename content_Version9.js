...
// Improved: Skips SVGs, logs all decisions, ensures correct fallback/apply logic.
...
function isSVG(img) {
  try {
    // Handles URLs with params/hash/fragments too
...
function oldNearestNeighborFallback(img) {
  ...
  if (style.getPropertyValue('display') === 'none') {
    console.log('[SincUpscale] Fallback skipped: display none', img.src);
    return false;
  }
  ...
  if (zoom > 1 && isInteger(zoom)) {
    ...
    restoreRendering(img);
    console.log('[SincUpscale] Fallback skipped: not integer upscale or axes mismatch', img.src, {width, height, naturalWidth: img.naturalWidth, naturalHeight: img.naturalHeight, x, y, zoom});
    return false;
  }
}
...
function processImages() {
  for (const img of document.querySelectorAll('img')) {
    ...
    if (!needsUpscaling(img)) {
      console.log('[SincUpscale] Not upscaled (size check):', img.src);
      img.dataset.sincUpscaled = "no";
      continue;
    }
    img.dataset.sincUpscaled = "true";
    console.log('[SincUpscale] Processing image:', img.src);
