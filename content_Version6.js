// --- Sinc/Lanczos Upscaling Extension with Nearest Neighbor Fallback ---
...
// Apply CSS for nearest neighbor scaling (pixelated)
function setNearestNeighbor(img) {
  img.style.imageRendering = "pixelated";
  img.style.msInterpolationMode = "nearest-neighbor"; // For older IE
  img.style.webkitTransform = "translateZ(0)"; // Sometimes helps on Chrome
  // You may also want to log:
  console.log("[SincUpscale] Applied nearest neighbor CSS fallback:", img.src);
}

// Sinc (Lanczos) upscaling with WebGL
async function replaceWithSincCanvas(img, radius = 3) {
  ...
  // Fallback: If not CORS safe, use nearest neighbor CSS scaling!
  if (!isCORSsafe(img)) {
    setNearestNeighbor(img);
    return;
  }
